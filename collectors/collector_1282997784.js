...
// 账号ID
const ACCOUNT_ID = '1282997784';

// 种子类型配置
const SEED_TYPE = '白萝卜 (Lv1) 1分钟';

// 智能推荐种植开关
const ENABLE_SMART_PLANT = true;

// 土地管理开关
const AUTO_UNLOCK_LAND = false;
const AUTO_UPGRADE_LAND = false;
...
function analyzeFriendLands(lands, myGid, friendName = '') {
  ...
  for (const land of lands) {
    ...
    if (phaseVal === PlantPhase.MATURE && plant.stealable) {
      ...
      // 检查是否在跳过的作物列表中（支持热更新）
      if (ACCOUNT_CONFIG.skipCrops && ACCOUNT_CONFIG.skipCrops.length > 0) {
        if (ACCOUNT_CONFIG.skipCrops.includes(plantId)) {
          debugInfo.push('土地' + id + ':跳过' + plantName);
          continue;
        }
      }
...
let ACCOUNT_CONFIG = {
  ...
  autoRefillOrganicFertilizer: false,
  ...
  skipCrops: [1020002,2020002,1020003,1020059],
  ...
  notificationEmail: 'window.apple@outlook.com',
  seedType: '白萝卜 (Lv1) 1分钟',
  enableSmartPlant: true,
  autoUnlockLand: false,
  autoUpgradeLand: false,
  enableRandomFriendInterval: false,
  // 白名单（JSON数组）
  stealWhitelist: [],
  helpWaterWhitelist: [],
  helpWeedWhitelist: [],
...
function startConnection() {
  ...
  sendLog('[启动] 开始连接服务器...');
  connect('03d3688f0ab8fbb3b9f711156ad2c65a', async () => {
    isConnected = true;
    sendLog('登录成功');
