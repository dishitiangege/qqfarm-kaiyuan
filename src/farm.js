...
function analyzeLands(lands, antiStealConfig = null) {
    const result = {
        harvestable: [], needWater: [], needWeed: [], needBug: [],
        growing: [], empty: [], dead: [],
        harvestableInfo: [],  // 收获植物的详细信息 { id, name, exp }
        needAntiSteal: [],  // 需要防偷菜处理的土地 { id, matureTime }
    };
    ...
    for (const land of lands) {
        ...
        // 检查是否需要防偷菜处理
        if (antiStealConfig && antiStealConfig.enabled && phaseVal < PlantPhase.MATURE) {
            const matureTime = getMatureTime(plant.phases);
            if (matureTime > 0) {
                ...
                if (timeToMature > 0 && timeToMature <= antiStealConfig.advanceTime) {
                    ...
                    if (debug) console.log(`    → 结果: 即将成熟，需防偷菜 (${timeToMature}s后成熟)`);
                    continue;
                }
            }
        }
    ...
    if (debug) {
        ...
        console.log(`  防偷菜: ${result.needAntiSteal.length} [${result.needAntiSteal.map(x => x.id).join(',')}]`);
        console.log('====================================');
        console.log('');
    }
 ...
 * 获取作物成熟时间
 */
function getMatureTime(phases) {
    if (!phases || phases.length === 0) return 0;
    
    // 找到成熟阶段
    for (const phase of phases) {
        if (phase.phase === PlantPhase.MATURE) {
            return toTimeSec(phase.begin_time);
        }
    }
    return 0;
}
...
let antiStealConfig = {
    enabled: false,
    advanceTime: 30,  // 默认提前30秒
};

// 设置防偷菜配置
function setAntiStealConfig(config) {
    antiStealConfig = {
        enabled: config?.enabled || false,
        advanceTime: Math.max(20, Math.min(120, config?.advanceTime || 30)),  // 限制在20-120秒
    };
}
 ...
 * 执行防偷菜操作：使用有机肥加速并收获
 */
async function processAntiSteal(needAntiStealLands) {
    if (!needAntiStealLands || needAntiStealLands.length === 0) return 0;
    
    const landIds = needAntiStealLands.map(l => l.id);
    log('防偷菜', `检测到 ${landIds.length} 块土地即将成熟，使用有机肥加速`);
    
    try {
        // 使用有机肥加速
        const fertilized = await fertilize(landIds, ORGANIC_FERTILIZER_ID);
        if (fertilized > 0) {
            log('防偷菜', `已为 ${fertilized} 块土地使用有机肥`);
            // 等待一小段时间让服务器处理
            await sleep(200);
            ...
            try {
                ...
                log('防偷菜', `已收获 ${landIds.length} 块土地`);
                return landIds.length;
            } catch (e) {
                logWarn('防偷菜', `收获失败: ${e.message}`);
            }
        } else {
            logWarn('防偷菜', '有机肥使用失败，可能库存不足');
        }
    } catch (e) {
        logWarn('防偷菜', `处理失败: ${e.message}`);
    }
    return 0;
}
...
async function checkFarm(userConfig = null) {
    const state = getUserState();
    ...
    try {
        ...
        if (userConfig) {
            setAntiStealConfig({
                enabled: userConfig.enableAntiSteal,
                advanceTime: userConfig.antiStealAdvanceTime,
            });
        }
        
        const status = analyzeLands(lands, antiStealConfig);
        ...
        if (status.needAntiSteal.length) statusParts.push(`防:${status.needAntiSteal.length}`);
        ...
        const hasWork = status.harvestable.length || status.needAntiSteal.length || status.needWeed.length || status.needBug.length
        ...
        if (status.needAntiSteal.length > 0) {
            const antiStealCount = await processAntiSteal(status.needAntiSteal);
            if (antiStealCount > 0) {
        ...
        if (harvestedLandIds.length > 0) {
            try {
                ...
                if (refreshedReply.lands && refreshedReply.lands.length > 0) {
                    const refreshedStatus = analyzeLands(refreshedReply.lands, antiStealConfig);
