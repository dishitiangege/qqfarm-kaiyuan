...
let itemInfoMap = new Map();  // id -> item
...
function loadConfigs() {
    ...
    try {
        ...
        if (fs.existsSync(itemInfoPath)) {
            ...
            console.log(`[配置] 已加载物品配置 (${itemInfoConfig.length} 种)`);
        }
    } catch (e) {
...
function getItemName(itemId) {
    ...
    // 回退到种子/植物/果实名称查询
    const plantBySeed = getPlantBySeedId(itemId);
    if (plantBySeed) return `${plantBySeed.name}种子`;
    
    const plantByFruit = getPlantByFruitId(itemId);
    if (plantByFruit) return `${plantByFruit.name}果实`;
...
// 可自动使用的物品ID缓存
let autoUsableItemIds = null;

/**
 * 获取可自动使用的物品ID集合
 * 类型11且can_use >= 1的物品（礼包类）
 * @returns {Set<number>}
 */
function getAutoUsableItemIds() {
    if (autoUsableItemIds) return autoUsableItemIds;
    
    autoUsableItemIds = new Set();
    for (const item of itemInfoMap.values()) {
        const itemType = Number(item.type);
        const canUse = Number(item.can_use || 0);
        const itemId = Number(item.id);
        if (itemType === 11 && canUse >= 1 && itemId > 0) {
            autoUsableItemIds.add(itemId);
        }
    }
