...
async function visitFriend(friend, totalActions, myGid, config = {}) {
    const { gid, name } = friend;
    const showDebug = DEBUG_FRIEND_LANDS === true || DEBUG_FRIEND_LANDS === name;

    // 经验上限停止帮忙配置（参考 new sample）
    const stopWhenExpLimit = config.stopWhenExpLimit === true;
    ...
    // 帮助操作: 只在有经验时执行（如果启用了 stopWhenExpLimit）
    // 参考 new sample 实现
    if (stopWhenExpLimit && !canGetHelpExp) {
        // 今日已达到经验上限后停止帮忙
    } else {
        // 除草 (10005) - 对应放草是 10003
        if (status.needWeed.length > 0) {
            const shouldHelp = !stopWhenExpLimit || canGetExpByCandidates([10005, 10003]);
            if (shouldHelp) {
                let ok = 0;
                for (const landId of status.needWeed) {
                    try {
                        await helpWeed(gid, [landId], stopWhenExpLimit);
                        ok++;
                    } catch (e) { /* ignore */ }
                    await sleep(100);
                }
        ...
        // 除虫 (10006) - 对应放虫是 10004
        if (status.needBug.length > 0) {
            const shouldHelp = !stopWhenExpLimit || canGetExpByCandidates([10006, 10004]);
            if (shouldHelp) {
                let ok = 0;
                for (const landId of status.needBug) {
                    try {
                        await helpInsecticide(gid, [landId], stopWhenExpLimit);
                        ok++;
                    } catch (e) { /* ignore */ }
                    await sleep(100);
                }
        ...
        // 浇水 (10007) - 对应收获是 10001
        if (status.needWater.length > 0) {
            const shouldHelp = !stopWhenExpLimit || canGetExpByCandidates([10007, 10001]);
            if (shouldHelp) {
                let ok = 0;
                for (const landId of status.needWater) {
                    try {
                        await helpWater(gid, [landId], stopWhenExpLimit);
                        ok++;
                    } catch (e) { /* ignore */ }
                    await sleep(100);
                }
...
async function checkFriends(config = {}) {
    ...
    try {
        ...
        if (config.useFixedFriendList && config.fixedFriendIds && config.fixedFriendIds.length > 0) {
            // 使用用户指定的固定好友ID列表
            friendsToVisit = config.fixedFriendIds.map(gid => ({ gid, name: `好友${gid}` }));
            log('好友', `使用固定好友列表，共 ${friendsToVisit.length} 人`);
        } else {
            // 原有逻辑：获取全部好友并筛选
            ...
            // 合并列表：优先好友在前
            friendsToVisit = [...priorityFriends, ...otherFriends];
        ...
        for (let i = 0; i < friendsToVisit.length; i++) {
            ...
            try {
                await visitFriend(friend, totalActions, state.gid, config);
            } catch (e) {
                if (showDebug) {
 ...
 */
async function friendCheckLoop(config = {}) {
    while (friendLoopRunning) {
        await checkFriends(config);
        if (!friendLoopRunning) break;
        await sleep(CONFIG.friendCheckInterval);
    }
}

function startFriendCheckLoop(config = {}) {
    ...
    friendCheckTimer = setTimeout(() => friendCheckLoop(config), 5000);
